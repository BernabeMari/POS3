    ...
    public class StockService : IStockService
    {
        ...
        public async Task<Stock> GetStockByNameAsync(string ingredientName)
        {
            ...
            var stock = await _context.Stocks
                .FirstOrDefaultAsync(s => s.ProductName.ToLower() == ingredientName.ToLower());
            ...
            if (stock != null)
            {
                _logger.LogInformation($"Found partial match for ingredient '{ingredientName}' with stock item '{stock.ProductName}'");
                return stock;
            }
            
            // No match found
            _logger.LogWarning($"No stock found for ingredient: {ingredientName}");
            return null;
        }
